    ...
    hardInteraction(cfg.getParameter<edm::ParameterSet>("hardInteraction"))    
{
  ...
  for (size_t i = 0; i < triggerDecision.l1_paths().size(); ++i) {
    ...
    l1_path_effs_bb.push_back(make_eff_pair(TString::Format("L1Path_%u_%s", unsigned(i), triggerDecision.l1_paths().at(i).c_str()), TString::Format("#varepsilon(%s) vs. mass", triggerDecision.l1_paths().at(i).c_str())));
    l1_path_effs_e.push_back(make_eff_pair(TString::Format("L1Path_%u_%s", unsigned(i), triggerDecision.l1_paths().at(i).c_str()), TString::Format("#varepsilon(%s) vs. mass", triggerDecision.l1_paths().at(i).c_str())));
  }
  ...
  for (size_t i = 0; i < triggerDecision.hlt_paths().size(); ++i) {
    ...
    hlt_path_effs_bb.push_back(make_eff_pair(TString::Format("HLTPath_%u_%s", unsigned(i), triggerDecision.hlt_paths().at(i).c_str()), TString::Format("#varepsilon(%s) vs. mass", triggerDecision.hlt_paths().at(i).c_str())));
    hlt_path_effs_e.push_back(make_eff_pair(TString::Format("HLTPath_%u_%s", unsigned(i), triggerDecision.hlt_paths().at(i).c_str()), TString::Format("#varepsilon(%s) vs. mass", triggerDecision.hlt_paths().at(i).c_str())));
  }
  
  l1_or_eff   = make_eff_pair("L1OrEff",   TString::Format("#varepsilon(%s) vs. mass", join(triggerDecision.l1_paths(),  std::string(" || ")).c_str()));
  l1_or_eff_bb   = make_eff_pair("L1OrEff",   TString::Format("#varepsilon(%s) vs. mass", join(triggerDecision.l1_paths(),  std::string(" || ")).c_str()));
  l1_or_eff_e   = make_eff_pair("L1OrEff",   TString::Format("#varepsilon(%s) vs. mass", join(triggerDecision.l1_paths(),  std::string(" || ")).c_str()));
  ...
  total_trig_eff_bb = make_eff_pair("TotalTrigEff", TString::Format("#varepsilon((%s) && (%s)) vs. mass", join(triggerDecision.l1_paths(), std::string(" || ")).c_str(), join(triggerDecision.hlt_paths(), std::string(" || ")).c_str()));
  total_trig_eff_e = make_eff_pair("TotalTrigEff", TString::Format("#varepsilon((%s) && (%s)) vs. mass", join(triggerDecision.l1_paths(), std::string(" || ")).c_str(), join(triggerDecision.hlt_paths(), std::string(" || ")).c_str()));
}
    ...
    for (pat::TriggerObjectStandAlone obj : *trigger_summary_src) { // note: not "const &" since we want to call unpackPathNames
        ...
        //obj.unpackFilterLabels(event, *triggerBits); // 92X
         ...
         for (unsigned h = 0; h < obj.filterLabels().size(); ++h) {
           if (checking_prescaled_path && (obj.filterLabels()[h] == pandf.prescaled_filter || obj.filterLabels()[h] == pandf.prescaled_filter_2) ){
           //FilterMatched[j] = 1;
           l3_mus.push_back(obj);
         }
           if (!checking_prescaled_path && (obj.filterLabels()[h] == pandf.filter || obj.filterLabels()[h] == pandf.filter_2) ){ 
           //FilterMatched[j] = 1;
           l3_mus.push_back(obj);
         }
