...
config.Data.outLFNDirBase = '/store/user/federica/Histos' 
...
'''
    ...
    for sample in samples:
       ...
       # str1 = 'switchHLTProcessName(process, "HLT")'
       # str2 = 'switchHLTProcessName(process, "%(hlt_process_name)s")' % sample.__dict__

        new_py = open('histos_2015.py').read()
        #new_py = new_py.replace(str1, str2)
        #print new_py.replace(str1, str2)
        # new_py += '\nswitchHLTProcessName(process, "%(hlt_process_name)s")\n' % sample.__dict__

        sample.pset = 'histos_mc_crab_%(name)s.py' % sample.__dict__
        open(sample.pset,'wt').write(new_py)

        sample.job = 'crab_histo_%(name)s.py' % sample.__dict__
        open(sample.job, 'wt').write(crab_cfg % sample.__dict__)
        if not just_testing:
            if do:
                print 'crab submit -c ' + sample.job
