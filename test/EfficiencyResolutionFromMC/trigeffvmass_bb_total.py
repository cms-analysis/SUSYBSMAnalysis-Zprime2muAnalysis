...
samples = ['dy120', 'dy200', 'dy400', 'dy800', 'dy1400', 'dy2300', 'dy3500', 'dy4500', 'dy6000']#, 'zp1500_c1', 'zp1750_c1', 'zp2000_c1', 'zp2250_c1', 'zp2500_c1', 'zp2750_c1', 'zp3000_c1']
#samples = ['dy50']
...
if 'vbtf' in sys.argv:
    ...
    plot_dir = 'plots/trigeffvsmassmctruth_bb_total'

if kind:
    kind = kind[0]
    plot_dir += '_' + kind
else:
    kind = 'normal_nomasswin'
...
types = ['AccNoPt', 'Acceptance', 'RecoWrtAcc', 'RecoWrtAccTrig', 'TotalReco']
#types = ['AccNoPt_bb', 'Acceptance_bb', 'RecoWrtAcc_bb', 'RecoWrtAccTrig_bb', 'TotalReco_bb', 'L1OrEff', 'HLTOrEff', 'TotalTrigEff']
types_ne = ['AccNoPt_ne', 'Acceptance_ne', 'RecoWrtAcc_ne', 'RecoWrtAccTrig_ne', 'TotalReco_ne', 'L1OrEff', 'HLTOrEff', 'TotalTrigEff']
types_pe = ['AccNoPt_pe', 'Acceptance_pe', 'RecoWrtAcc_pe', 'RecoWrtAccTrig_pe', 'TotalReco_pe', 'L1OrEff', 'HLTOrEff', 'TotalTrigEff']
...
#        continue
    if not totals_histos.has_key(t):
        totals_histos[t] = ROOT.TH1F(t + '_totals_num', '', 6100, 0, 6100), ROOT.TH1F(t + '_totals_den', '', 6100, 0, 6100)
...
def do_replace_or_add(name, samples, which, add=False):
    ...
    eff = binomial_divide(num, den)
    eff.Draw('AP')
    eff.GetXaxis().SetLimits(80, 6000)
    ...
    eff.GetYaxis().SetTitle('Acceptance' if which in ['AccNoPt_bb', 'Acceptance_bb'] else 'Efficiency')
...
#lg.AddEntry(Total, 'Level-1+HLT', 'LPE')
##L1Or.GetXaxis().SetRangeUser(80, 3000)
##L1Or.GetYaxis().SetRangeUser(0.93, 1.005)
##L1Or.Draw('AP')
##HLTOr.Draw('P same')
#Total.GetXaxis().SetRangeUser(80, 6000)
...
##lg.AddEntry(RecoWrtAccTrig, 'w.r.t. triggered events in acceptance', 'LPE')
lg.SetMargin(0.13)
lg.AddEntry(RecoWrtAcc, 'w.r.t. events in BB acceptance', 'LPE')
lg.AddEntry(TotalReco, 'total', 'LPE')
RecoWrtAcc.GetXaxis().SetRangeUser(0, 6000)
RecoWrtAcc.GetYaxis().SetRangeUser(-0.5, 1.0)
RecoWrtAcc.GetYaxis().SetTitleOffset(1.0);
RecoWrtAcc.Draw('AP')
##RecoWrtAccTrig.Draw('P same')
TotalReco.Draw('P same')
fitwindow = 180,5500
#RecoWrtAcc.Fit('pol1', 'VR', '', *fitwindow)
fcn = ROOT.TF1('fcn', '[0] + [1]/(x+[2]) - [3]*x', *fitwindow)
#fcn.SetParLimits(2,0.000012,0.00002)
#fcn.SetParLimits(0,0.95,1.2)
#fcn.SetParLimits(1,-30,-1)
#fcn.SetParLimits(3,1e7,1.e14)
fcn.SetParNames("a", "b", "c", "d")
fcn.SetLineColor(TotalReco.GetLineColor())
lg.AddEntry(fcn, 'fit to a + b/(m+c) - d*m for m in 200-5500 GeV', 'L')
#lg.SetTextFont(42)
...
t.SetFillStyle(0)
t.Draw()
t2 = ROOT.TPaveLabel(0.58, 0.89, 0.86, 0.98, 'BB category', 'brNDC')
t2.SetTextFont(42)
t2.SetTextSize(0.39)
t2.SetBorderSize(0)
t2.SetFillColor(0)
