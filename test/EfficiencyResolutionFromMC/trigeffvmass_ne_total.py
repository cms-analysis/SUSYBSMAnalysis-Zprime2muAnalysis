...
if 'vbtf' in sys.argv:
    ...
    plot_dir = 'plots/trigeffvsmassmctruth_ne_total'
...
#types = ['AccNoPt_ne', 'Acceptance_ne', 'RecoWrtAcc_ne', 'RecoWrtAccTrig_ne', 'TotalReco_ne', 'L1OrEff', 'HLTOrEff', 'TotalTrigEff']
...
if make_individual_effs:
    do_overlay('dy',dy,'Acceptance_ne')
    do_overlay('zp',zp,'Acceptance_ne')
...
def do_replace_or_add(name, samples, which, add=False):
    ...
    eff.GetYaxis().SetTitle('Acceptance' if which in ['AccNoPt_ne', 'Acceptance_ne'] else 'Efficiency')
...
lg.AddEntry(RecoWrtAcc, 'w.r.t. events in E- acceptance', 'LPE')
lg.AddEntry(TotalReco, 'total', 'LPE')
RecoWrtAcc.GetXaxis().SetRangeUser(0, 6000)
RecoWrtAcc.GetYaxis().SetRangeUser(-1.5, 1.)
...
fcn = ROOT.TF1('fcn', '[0] + [1]/(x)**3 - (x*x/[2])', *fitwindow)
#fcn.SetParLimits(2,0,1000)
#fcn.SetParLimits(0,0.22,0.225)
fcn.SetParLimits(1,-9e9,-1.e5)
fcn.SetParLimits(2,1e7,1.e14)
fcn.SetParNames("a", "b", "c")
fcn.SetLineColor(TotalReco.GetLineColor())
lg.AddEntry(fcn, 'fit to a + b/pow(m, 3) - m^2/c for m in 200-5500 GeV', 'L')
#lg.SetTextFont(42)
...
t.SetFillStyle(0)
t.Draw()
t2 = ROOT.TPaveLabel(0.58, 0.89, 0.86, 0.98, 'BE- + EE category', 'brNDC')
t2.SetTextFont(42)
t2.SetTextSize(0.39)
t2.SetBorderSize(0)
t2.SetFillColor(0)
