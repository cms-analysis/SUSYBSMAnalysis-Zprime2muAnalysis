...
if 'vbtf' in sys.argv:
    ...
    plot_dir = 'plots/trigeffvsmassmctruth_pe_total'
...
#types = ['AccNoPt_pe', 'Acceptance_pe', 'RecoWrtAcc_pe', 'RecoWrtAccTrig_pe', 'TotalReco_pe', 'L1OrEff', 'HLTOrEff', 'TotalTrigEff']
...
if make_individual_effs:
    do_overlay('dy',dy,'Acceptance_pe')
    do_overlay('zp',zp,'Acceptance_pe')
...
def do_replace_or_add(name, samples, which, add=False):
    ...
    eff.GetYaxis().SetTitle('Acceptance' if which in ['AccNoPt_pe', 'Acceptance_pe'] else 'Efficiency')
...
t.SetFillStyle(0)
t.Draw()
t2 = ROOT.TPaveLabel(0.58, 0.89, 0.86, 0.98, 'BE+ category', 'brNDC')
t2.SetTextFont(42)
t2.SetTextSize(0.39)
t2.SetBorderSize(0)
t2.SetFillColor(0)
